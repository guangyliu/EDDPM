...
with torch.no_grad():
    ...
    os.makedirs(f'./samples/{args.dataset}/interpolation_ddpm_T={args.time_steps}_N={args.num_samples}', exist_ok=True)
    ...
    for batch1, batch2 in tqdm(zip(dataloader_subset1, dataloader_subset2), total=len(dataloader_subset1)):
        ...
        for mix_ratio in [0.2, 0.4]:
            ...
            for j, img in enumerate(pred_img):
                os.makedirs(f'./samples/{args.dataset}/interpolation_ddpm_T={args.time_steps}_N={args.num_samples}/batch_{count+j}', exist_ok=True)
                plt.imsave(f'./samples/{args.dataset}/interpolation_ddpm_T={args.time_steps}_N={args.num_samples}/batch_{count+j}/interpolation_{mix_ratio}_0.9.png', img.cpu().permute([1, 2, 0]).numpy())
            
        count += len(pred_img)

    # all_imgs = np.concatenate(all_imgs, axis=0)
    # np.save(f'./samples/interpolation_ddpm_T={args.time_steps}_N={args.num_samples}/numpys/all_imgs.npy', all_imgs)