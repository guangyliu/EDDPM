...
class LitModel(pl.LightningModule):
    def __init__(self, conf: TrainConfig):
        ...
        self.model.var_t1 = self.sampler.posterior_variance[1]
        self.ema_model.var_t1 = self.sampler.posterior_variance[1]
        ...
        if hasattr(conf, "latent_model"):
            print(f'loading latent model ... {conf.latent_model.name}')
            latent_state = torch.load(conf.latent_model.path, map_location='cpu')
            try:
                print('step:', state['global_step'])
                self.load_state_dict(latent_state['state_dict'], strict=False)
            except:
                self.model.load_state_dict(latent_state.state_dict())
            
        if hasattr(conf, 'autoenc'):
            print(f'loading autoenc model ... {conf.autoenc.name}')
            autoenc_state = torch.load(conf.autoenc.path, map_location='cpu')
            try:
                print('step:', state['global_step'])
                self.load_state_dict(autoenc_state['state_dict'], strict=False)
            except:
                self.model.load_state_dict(autoenc_state.state_dict())
        ...
        if conf.latent_infer_path is not None:
            ...
            # self.conds_std = None
            #### add for joint training
            if state is not None:
                print('loading latent stats ...')
                # state = torch.load('./checkpoints/ffhq128_autoenc/latent.pkl')
                # self.conds = self.infer_whole_dataset()
                self.register_buffer('conds_mean', state['state_dict']['conds_mean'])
                self.register_buffer('conds_std', state['state_dict']['conds_std'])
            # self.register_buffer('conds_mean', torch.zeros_like(state['conds_mean'][None, :]))
            # self.register_buffer('conds_std', torch.ones_like(state['conds_std'][None, :]))
    
        self.best_fid_score = 1e9
        self.best_fid_ema_score = 1e9
    ...
    def train_dataloader(self):
        ...
        print("require_dataset_infer:", self.conf.train_mode.require_dataset_infer())
    ...
    def training_step(self, batch, batch_idx):
        ...
        with amp.autocast(False):
            ...
            if self.conf.train_mode.require_dataset_infer():
                ...
                cond = batch[0]  # pre-calculated z_0
                if self.conf.latent_znormalize:
                    cond = (cond - self.conds_mean.to(
                        self.device)) / self.conds_std.to(self.device)
            else:
                imgs, idxs = batch['img'], batch['index']  # b x ch x img x img (128x3x128x128)
                # print(f'(rank {self.global_rank}) batch size:', len(imgs))
                x_start = imgs
            if self.conf.train_mode.is_joint(): # include two stage
                t, weight = self.T_sampler.sample(len(x_start), x_start.device) # t \in [0,999] predict xt

                losses_ = self.sampler.training_losses(model=self.model,
                                                      x_start=x_start,
                                                      t=t) 

                # dict_key:  x_t: xt, cond: z0, pred_xstart: x0*, mse, loss, mean
                cond = losses_['cond']  
                cond = (cond - self.conds_mean.to(
                        self.device)) / self.conds_std.to(self.device) # cond z_sem is sampled from N (enc(x), var) 
                mean = losses_['mean']
                mean = (mean - self.conds_mean.to(
                        self.device)) / self.conds_std.to(self.device)
                ...
                latent_losses = self.latent_sampler.training_losses(
                    model=self.model.latent_net, x_start=cond, t=t, mean=mean)  # dict_key: x_t: zt, cond: None, pred_xstart: z0*, mse, loss
                joint_loss = self.conf.ldm_weight * latent_losses['loss'] + (2-self.conf.ldm_weight) * losses_['loss']
                # joint_loss = self.conf.ldm_weight * latent_losses['loss'] +  losses_['loss']
                
                losses={
                    'latent': latent_losses['loss'],
                    'vae': losses_['loss'],
                    'loss': joint_loss,
                    'norm': mean.norm(2,dim=1)
                }
            elif self.conf.train_mode == TrainMode.diffusion:
                """
                main training mode!!!
                """
            ...
            if self.num_samples % (self.conf.batch_size * 50) == 0:
                for key in ['loss', 'vae', 'latent', 'mmd', 'chamfer', 'arg_cnt','norm']:
                    if key in losses:
                ...
                if self.global_rank == 0:
                    self.logger.experiment.add_scalar('loss', losses['loss'],
                                                    self.num_samples)
                    for key in ['vae', 'latent', 'mmd', 'chamfer', 'arg_cnt','norm']:
                        if key in losses:
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx: int,
        ...
        if self.is_last_accum(batch_idx):
            ...
            fid_score, fid_ema_score = self.evaluate_scores()

            # print("###############################")
            # print("fid_score:", fid_score, "ema fid score:", fid_ema_score)
            # print("###############################")

            ckpt_dir = os.path.join(self.conf.logdir, 'fid_ckpt')
            os.makedirs(ckpt_dir, exist_ok=True)
            if fid_score is not None and fid_score < self.best_fid_score:
                self.best_fid_score = fid_score
                torch.save(self.model, os.path.join(ckpt_dir, "best_fid_model.ckpt"))

            if fid_ema_score is not None and fid_ema_score < self.best_fid_ema_score:
                self.best_fid_ema_score = fid_ema_score
                torch.save(self.ema_model, os.path.join(ckpt_dir, "best_fid_ema_model.ckpt"))
    ...
    def log_sample(self, x_start):
        ...
        def do(model,
               ...
               interpolate=False):
            last_batch_size = len(x_start)
            model.eval()
            with torch.no_grad():
                all_x_T = self.split_tensor(self.x_T[:last_batch_size])
    ...
    def evaluate_scores(self):
        ...
        if self.conf.eval_ema_every_samples > 0 and self.num_samples > 0 and is_time(
            ...
            ema_fid_score = fid(self.ema_model, '_ema')
            # it's too slow
            # lpips(self.ema_model, '_ema') 

        return fid_score, ema_fid_score
...
def train(conf: TrainConfig, gpus, nodes=1, mode: str = 'train'):
    ...
    if mode == 'train':
        ...
        model.model = model.ema_model = torch.load(eval_path, map_location='cpu')
