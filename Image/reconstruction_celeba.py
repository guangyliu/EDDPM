...
with torch.no_grad():
    ...
    model.model = torch.load(f'./checkpoints/diffae_ckpt/v3/celeba64d2c_2gpu_b100_l0_lc_w1_scale_ft/fid_ckpt/best_fid_ema_model.ckpt', map_location='cpu')
    ...
    os.makedirs(f'./samples/celeba_reconstructions_best_fid_ema_T={args.time_steps}_N={args.num_samples}', exist_ok=True)
    os.makedirs(f'./samples/celeba_reconstructions_best_fid_ema_T={args.time_steps}_N={args.num_samples}/images', exist_ok=True)
    ...
    for batch in tqdm(dataloader):
        ...
        if os.path.exists(f'./samples/celeba_reconstructions_best_fid_ema_T={args.time_steps}_N={args.num_samples}/images/image_{count+j}.png'): 
        ...
        for j, img in enumerate(pred_img):
            plt.imsave(f'./samples/celeba_reconstructions_best_fid_ema_T={args.time_steps}_N={args.num_samples}/images/image_{count+j}.png', img.cpu().permute([1, 2, 0]).numpy())
