...
with torch.no_grad():
    ...
    if args.dataset == 'ffhq':
        ...
        conf.pretrain = PretrainConfig(
            name='200M',
            path=f'ckpts/bedroom128_autoenc/last.ckpt', # ae + ldm
            # path=f'checkpoints/ffhq128_autoenc/last_aeonly.ckpt', # only ae
            # path=f'checkpoints/ciai_bk/last.ckpt'
            # path=f'checkpoints/diffae_ckpt/v3/celeba64d2c_2gpu_b100_l0_lc_w1_scale_ft/fid_ckpt/best_fid_model.ckpt'
        )
    ...
    if args.dataset == 'ffhq':
        ...
        if args.ckpt == 'best_ema':
            model.model = torch.load("checkpoints/diffae_ckpt/v3/bedrooms_4gpu_b100_l0_lc_w1.9_scale_ft_teval100/fid_ckpt/best_fid_ema_model.ckpt", map_location='cpu')
        elif args.ckpt == 'bs':
            pass
    ...
    for batch1, batch2 in tqdm(zip(dataloader_subset1, dataloader_subset2), total=len(dataloader_subset1)):
        ...
        if os.path.exists(f'./samples/{args.dataset}/{folder_str}_T={args.time_steps}_N={args.num_samples}/batch_{count+len(original_img_1) - 1}/interpolation_1.0.png'):
