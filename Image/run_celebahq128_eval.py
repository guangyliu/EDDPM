...
if __name__ == '__main__':
    gpus = [2]
    conf = celeba128_autoenc_joint_eval()
    # conf.latent_infer_path = 'checkpoints/celeba128_eval/latent.pkl'
    conf.batch_size = 50 # 100
    conf.ldm_weight = 1.0  # w,   2-w
    conf.name = 'celeba128_eval' #'v6_b100_l0_lc_w1_scale_scldm' #4gpu_noscale_noln_d3_w'+str(conf.ldm_weight)+'_b200_ll_l2l2_detach1_ft'  # ckpt name
    conf.pretrain = PretrainConfig(
        name='130M',
        # path=f'checkpoints/ffhq128_autoenc/last.ckpt', # ae + ldm
        # path=f'checkpoints/ffhq128_autoenc/last_aeonly.ckpt', # only ae
        # path=f'checkpoints/ciai_bk/last.ckpt'
        path=f'checkpoints/diffae_ckpt/v3/v6_b100_l0_lc_w1_scale_ft/fid_ckpt/best_fid_model.ckpt'
    )
    
    conf.eval_programs = ['infer']
    conf.eval_path = f'checkpoints/diffae_ckpt/v3/v6_b100_l0_lc_w1_scale_ft/fid_ckpt/best_fid_model.ckpt'
    # conf.pretrain = None
    # print(conf.lr)
    # conf.lr = 0.00005
