...
if __name__ == '__main__':
    gpus = [eval(x) for x in sys.argv[2].split(",")]
    conf = bedroom128_autoenc_joint()
    conf.latent_infer_path = None
    conf.batch_size = 25 # 100 for 4 gpus
    conf.ldm_weight = eval(sys.argv[1])  # w,   2-w
    conf.T_eval = 100
    conf.latent_T_eval = 100
    conf.lr = 4e-6
    conf.name = f'bedrooms_4gpu_b100_l0_lc_w{sys.argv[1]}_scale_ft_teval100' #'v6_b100_l0_lc_w1_scale_scldm' #4gpu_noscale_noln_d3_w'+str(conf.ldm_weight)+'_b200_ll_l2l2_detach1_ft'  # ckpt name
