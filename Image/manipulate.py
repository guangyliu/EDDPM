...
os.makedirs(f'./imgs_manipulated/bs', exist_ok=True)
os.makedirs(f'./imgs_manipulated/ori', exist_ok=True)
os.makedirs(f'./imgs_manipulated/ours', exist_ok=True)
...
for i in range(10):
    ...
    for cls_name in CelebHQAttrDataset.cls_to_id:
        
        if cls_name not in ["Goatee", "Bushy_Eyebrows", "Black_Hair", "Blond_Hair"]: continue
        ...
        for extent in [0.1, 0.3]:
            ...
            plt.imsave(f"./imgs_manipulated/ori/{cls_name}_{i}_{extent}.png", ori[0].permute(1, 2, 0).cpu().numpy())
            plt.imsave(f"./imgs_manipulated/bs/{cls_name}_{i}_{extent}.png", img_bs[0].permute(1, 2, 0).cpu().numpy())
            plt.imsave(f"./imgs_manipulated/ours/{cls_name}_{i}_{extent}.png", img[0].permute(1, 2, 0).cpu().numpy())
