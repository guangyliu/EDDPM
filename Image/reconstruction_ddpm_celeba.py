...
with torch.no_grad():
    ...
    os.makedirs(f'./samples/celeba_reconstructions_ddpm_T={args.time_steps}_N={args.num_samples}', exist_ok=True)
    os.makedirs(f'./samples/celeba_reconstructions_ddpm_T={args.time_steps}_N={args.num_samples}/images', exist_ok=True)
    os.makedirs(f'./samples/celeba_reconstructions_ddpm_T={args.time_steps}_N={args.num_samples}/numpys', exist_ok=True)
    ...
    for batch in tqdm(dataloader):
        ...
        if os.path.exists(f'./samples/celeba_reconstructions_ddpm_T={args.time_steps}_N={args.num_samples}/images/image_{count+j}.png'): 
        ...
        for j, img in enumerate(pred_img):
            plt.imsave(f'./samples/celeba_reconstructions_ddpm_T={args.time_steps}_N={args.num_samples}/images/image_{count+j}.png', img.cpu().permute([1, 2, 0]).numpy())
