...
with torch.no_grad():
    ...
    os.makedirs(f'./samples/{args.dataset}/original_imgs_256', exist_ok=True)
    ...
    for batch in tqdm(dataloader):
        ...
        for j in range(len(original_img)):
            if not os.path.exists(f'./samples/{args.dataset}/original_imgs_256/original_image_{count+j}.png'):
                plt.imsave(f'./samples/{args.dataset}/original_imgs_256/original_image_{count+j}.png', original_img[j].cpu().permute([1, 2, 0]).numpy())
