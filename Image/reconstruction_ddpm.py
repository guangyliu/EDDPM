...
parser.add_argument('--dataset', default='ffhq', type=str)
...
with torch.no_grad():
    ...
    # model.model = 
    if args.dataset == 'ffhq':
        state = torch.load(f"checkpoints/ffhq128_ddpm_130M/last.ckpt", map_location='cpu')
    elif args.dataset == 'bedroom':
        state =  torch.load(f"ckpts/bedroom_ddpm_130M/last.ckpt", map_location='cpu')
    elif args.dataset == 'horse':
        state =  torch.load(f"ckpts/horse_ddpm_130M/last.ckpt", map_location='cpu')
    ...
    os.makedirs(f'./samples/reconstructions_ddpm_T={args.time_steps}_N={args.num_samples}', exist_ok=True)
    os.makedirs(f'./samples/reconstructions_ddpm_T={args.time_steps}_N={args.num_samples}/images', exist_ok=True)
    os.makedirs(f'./samples/reconstructions_ddpm_T={args.time_steps}_N={args.num_samples}/numpys', exist_ok=True)
    ...
    for batch in tqdm(dataloader):
        ...
        if os.path.exists(f'./samples/reconstructions_ddpm_T={args.time_steps}_N={args.num_samples}/images/image_{count+j}.png'): 
        ...
        for j, img in enumerate(pred_img):
            plt.imsave(f'./samples/reconstructions_ddpm_T={args.time_steps}_N={args.num_samples}/images/image_{count+j}.png', img.cpu().permute([1, 2, 0]).numpy())
