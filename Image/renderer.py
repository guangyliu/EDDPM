...
def render_uncondition(conf: TrainConfig,
    ...
    device = x_T.device
    if conf.train_mode == TrainMode.diffusion:  # for ddpm
        assert conf.model_type.can_sample()
        return sampler.sample(model=model, noise=x_T)
    elif conf.train_mode.is_latent_diffusion():   # for autoencoders
        model: BeatGANsAutoencModel
...
def render_condition(
    ...
    if conf.train_mode == TrainMode.diffusion or conf.train_mode == TrainMode.joint:
