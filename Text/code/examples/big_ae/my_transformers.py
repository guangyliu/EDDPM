...
class LlamaModelForVAE(LlamaModel, ABC):
    def __init__(self, config, latent_size=64, latent_as_gpt_emb=True):
        super().__init__(config)
        self.latent_size = latent_size
        self.linear = nn.Linear(self.latent_size, config.hidden_size * config.num_hidden_layers,
                                bias=False)  # different latent vector for each layer
        # if latent_as_gpt_emb:
        self.linear_emb = nn.Linear(self.latent_size, config.hidden_size,
                                    bias=False)  # share the same latent vector as the embeddings
    ...
    def split_head(self, tensor, num_heads, attn_head_size):
        """
    ...
    def forward(self,
        ...
        if past_key_values is None:
            ...
            if latent_as_gpt_emb:
                past_emb = self.linear_emb(past_key_values)  # used as embeddings to add on other three embeddings
                inputs_embeds = self.embed_tokens(input_ids) + past_emb.unsqueeze(1)
            else:
                inputs_embeds = self.embed_tokens(input_ids)
            if latent_as_gpt_memory:
                past_key_values = self.linear(past_key_values)   # batch x (n_layer * hidden_size)
                
                num_att = past_key_values.shape[-1] // (self.config.hidden_size * self.config.num_hidden_layers)
                ### ori
                if num_att == 1:
                    past_split = torch.split(past_key_values.unsqueeze(1), self.config.hidden_size, dim=2) # tuple: 49  tensor: batch x 1 x hidden_size
                else:
                ...
                # import ipdb
                # ipdb.set_trace()
                past_split = [self.split_head(past, self.config.num_attention_heads, self.config.hidden_size//self.config.num_attention_heads) # list: 49  tensor: batch x n_head x 1 x (hidden/n_head)
                            for past in past_split]
                
                # past_split = [past.view(past_split[0].shape[0],self.config.num_attention_heads,num_att,-1) # list: 49  tensor: batch x n_head x 1 x (hidden/n_head)
                            # for past in past_split]
                
                past_key_values = None #list(zip(past_split, past_split))
        ...
        return super().forward(inputs_embeds=inputs_embeds, past_key_values=past_key_values,
                            attention_mask=attention_mask,
                            position_ids=position_ids ,output_attentions=output_attentions,output_hidden_states=output_hidden_states,)
            
class LlamaForLatentConnector(LlamaPreTrainedModel, ABC):
    def __init__(self, config, latent_size=64, latent_as_gpt_emb=True, latent_as_gpt_memory=True):
        super().__init__(config)
        self.transformer = LlamaModelForVAE(config, latent_size=latent_size, latent_as_gpt_emb=latent_as_gpt_emb)
