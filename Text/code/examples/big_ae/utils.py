...
class MultipleFiles_DataLoader(object):
    def __init__(self, file_path, batch_size, max_seq_length, tokenizer, args, bucket=100, shuffle=True,
        ...
        self.cached_features_file = os.path.join(self.file_path,
                                                 f'v2_cached_lm_gpt_bert_32_bookcorpus_split_{self.file_idx}.json')
...
class BucketingMultipleFiles_DataLoader(object):
    def __init__(self, file_path, batch_size, max_seq_length, tokenizer, args, bucket=100, shuffle=True):
        ...
        self.file_idx = 0
        self.cached_features_file = os.path.join(self.file_path,
                                                 f'cached_lm_gpt_bert_32_debug_{self.file_idx}.json')
        self.dataset = PreparedTokenDataset(tokenizer, self.args, self.cached_features_file,
                                            block_size=self.args.block_size)
        self.num_examples = len(self.dataset.examples)
        self.num_batches = self.num_examples // batch_size
...
class TokenDataset(Dataset):
    ...
    def collate(examples):
        ...
        input_ids_gpt = pad_sequence([torch.tensor(f['gpt2_token'], dtype=torch.long) for f in examples],
                                     batch_first=True, padding_value=gpt2_pad_token)
        try:
            token_lengths = torch.tensor([[int(f['bert_token_length']), int(f['gpt2_token_length'])] for f in examples],
                                         dtype=torch.long)
        except:
    ...
    def _read_corpus_natural_split(self, fname, label, max_length, block_size, args):
        ...
        with open(fname) as fin:
            ...
            for iii, line in pbar:
                pbar.set_description("Processing %s " % str(round(iii/7080.,4)))
                if label:
    ...
    def _read_corpus_natural_split_save(self, fname, label, max_length, block_size, args, cached_features_file):
        data = []
        labels = [] if label else None
        dropped = 0
        count = 0
...
def calc_ppl(model, tokenizer, generate_text):
    if model == None:
        from transformers import GPT2TokenizerFast, GPT2LMHeadModel
        model_id = '/home/guangyi.liu/LatentOps/classifiers/gpt2_yelp'
        model = GPT2LMHeadModel.from_pretrained(model_id).cuda()
        tokenizer = GPT2TokenizerFast.from_pretrained(model_id)
    pre = '' if generate_text[0][-1] == '\n' else '\n'
    encodings = tokenizer((pre+'\n\n').join(generate_text), return_tensors='pt')
    ...
    return ppl.item()
def calc_sbleu(list_of_references):
    if isinstance(list_of_references[0],str):
         tmp_list = [text.split() for text in list_of_references]
         list_of_references = tmp_list
    sbleu = []
    ...
    for i in range(num_all):
        ...
        bleu_ = sentence_bleu(refs, list_of_references[i], (0.25,0.25,0.25,0.25),smoothing_function=SmoothingFunction().method1)
